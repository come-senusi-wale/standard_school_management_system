<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        if (empty($addmission_number) || empty($image)) {
            ...
            header("location: ../student_image_editing_form.php?result=$out");
            exit();
        ...
        $query_two = "SELECT * FROM student_registration_table WHERE addmission_num = '$addmission_number'";
        ...
        if ($num < 1) {
            ...
            if (!in_array($image_exten, $allow)) {
                ...
                $image_path = '../../../image/student/'.$table_image;
                ...
                if ($image_path) {
                    ...
                    if (!$location) {
                        ...
                        $query = "UPDATE student_registration_table SET image = '$image_pass' WHERE addmission_num = '$addmission_number'";
                        $query_run = mysqli_query($conn, $query);

                        if ($query_run) {
                            
                            $out = 'image successfully updated';
                            header("location: ../student_image_editing_form.php?correct=$out");
                        }else{
