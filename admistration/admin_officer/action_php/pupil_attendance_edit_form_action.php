<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        if (empty($session) || empty($term) || empty($class) || empty($emial_status) || empty($user_name) || empty($attendance_status)) {
            ...
            header("location: ../pupil_attendance_edit_editing_form.php?result=$output&&id=$id");
            ...
            if (!preg_match($session_reg, $session)) {
                ...
                if ($num_tree < 1) {
                    ...
                    if ($check_four < 1) {
                        ...
                        $query = "SELECT * FROM pupil_attendance_creation_table";
                        ...
                        if ($num > 0) {
                            ...
                            if ($check > 0) {
                                ...
                                $query_two = "UPDATE pupil_attendance_creation_table SET email_status = '$emial_status', class = '$class', term = '$term', session = '$session', user_name = '$user_name', attendance_status = '$attendance_status' WHERE id = '$id'";
                                $query_run_two = mysqli_query($conn, $query_two);

                                if ($query_run_two) {
                                    
                                    $output = 'data succesfully updated';
                                    header("location: ../pupil_attendance_edit_editing_form.php?correct=$output&&id=$id");
