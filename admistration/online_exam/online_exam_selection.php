<?php
    ...
    if (!isset($_SESSION['addmission_num'])) {
    ...
    $query = "SELECT * FROM school_online_exam_creation_table WHERE session = '$session' AND term = '$term' AND class = '$class' AND exam_status = 'started'";
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        if (data == 'correct') {
                            ...
                            window.location.assign(`student_taken_online_exam_page.php?class=${classe}&term=${term}&duration=${duration}&session=${session}&mark=${mark}&exam_id=${exam_id}&total_question=${total_question}&subject=${subject}&type=${type}`);
