<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        if (empty($session) || empty($term)) {
            ...
            header("location: pupil_school_online_detail_form.php?fail=$output");
            exit();
            ...
            if (!preg_match($session_reg, $session)) {
                ...
                $query = "SELECT * FROM pupil_school_online_exam_creation_table WHERE session = '$session' AND term = '$term'";
...
<html lang="en">
    <head>
        ...
        <title>pupils school online exam detail</title>
    ...
    <body>
        ...
        <div class="reg_header">
            <h2>pupils school online exam detail</h2>
                ...
                <table>
                    ...
                    <?php
                        ...
                        while ($row = mysqli_fetch_array($query_run)) {
                            ...
                            $edit = '<a href="pupil_school_online_exam_edit_editing_form.php?id='.$id.'&exam_id='.$exam_id.'" class="edit_btn" id="edit'.$id.'" data-id="'.$id.'">edit</a>';

                            $add_question ='<a href="pupil_school_online_exam_question_add_form.php?id='.$id.'&&exam_id='.$exam_id.'&&class='.$class.'" class="view_btn" id="view'.$id.'" data-id="'.$id.'">add </a>';


                            $delete = '<button type="button" class="delete_btn" id="delete'.$id.'" data-id="'.$id.'" data-exam_id="'.$exam_id.'" data-class="'.$class.'">delete</button>';

                            $view_question ='<a href="pupil_school_online_exam_question_detail_view.php?id='.$id.'&&exam_id='.$exam_id.'&&class='.$class.'" class="view_btn" id="view'.$id.'" data-id="'.$id.'">view</a>';

                            $change_status ='<a href="pupil_school_online_exam_change_status.php?id='.$id.'&&exam_id='.$exam_id.'&&class='.$class.'" class="view_btn" id="view'.$id.'" data-id="'.$id.'">change</a>';
            ...
            $(document).on('click', '.delete_btn', function(event){
                ...
                if (confirm('are you sure want to delete this exam')) {
                    ...
                    $.ajax({
                        ...
                        data: {action: 'pupil delete online examination', id, exam_id, classe},
                        method: 'POST',
                        dataType: 'text',
                        beforeSend: function(){
